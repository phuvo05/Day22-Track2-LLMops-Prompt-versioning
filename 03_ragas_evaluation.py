...
NOTE: This step takes ~15-20 minutes due to RAGAS making many LLM calls.
"""
...
# ── 4. Prompt templates (V1: concise, V2: structured) ─────────────────────────
SYSTEM_V1 = (
...
# ── 5. LLM and Embeddings factory ──────────────────────────────────────────────
def create_llm(model: str = None, temperature: float = 0.0) -> ChatOpenAI:
    return ChatOpenAI(
...
def build_vectorstore():
    ...
    if not kb_path.exists():
        raise FileNotFoundError(f"Knowledge base not found at {kb_path}. Please create it first.")
...
def run_ragas_eval(rag_results: list, version: str) -> dict:
    ...
    """
    print(f"\nRunning RAGAS evaluation for prompt {version} (this may take a while)...")
    ...
    for key in ["faithfulness", "answer_relevancy", "context_recall", "context_precision"]:
        raw = result[key]
        valid_scores = [v for v in raw if v is not None and not np.isnan(v)]
        scores[key] = float(np.mean(valid_scores)) if valid_scores else 0.0

    print(f"\n  Scores for prompt {version}:")
...
def main():
    ...
    print("\n[1/5] Building vectorstore from knowledge base...")
    ...
    print("\n[4/5] Running RAGAS evaluation on V1 outputs...")
    v1_scores = run_ragas_eval(v1_results, "v1")

    print("\n[5/5] Running RAGAS evaluation on V2 outputs...")
    ...
    for metric in ["faithfulness", "answer_relevancy", "context_recall", "context_precision"]:
        ...
        if metric == "faithfulness":
            if s1 >= 0.8 or s2 >= 0.8:
                flag = " *"
            print(f"  {metric:30s}: V1={s1:.4f}  V2={s2:.4f}  {winner}{flag}")
        else:
    ...
    if best_faith >= 0.8:
        ...
        print(f"\n  Below target (best faithfulness = {best_faith:.4f}). "
              "Try adjusting chunking or prompts.")
